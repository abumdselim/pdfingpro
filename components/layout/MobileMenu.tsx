...
export default function MobileMenu({ open, onClose, onOpenSearch, t }: MobileMenuProps) {
        ...
        <nav className="flex-1 overflow-y-auto touch-scroll px-4 py-5">
          {onOpenSearch && (
            <button
              ...
              className={cn(
                "mb-4 flex w-full items-center gap-3 rounded-xl border border-slate-200/80 dark:border-slate-700/80",
                "bg-slate-50/80 dark:bg-slate-800/60 px-3.5 py-3.5 text-left transition-colors",
                "hover:border-[#1461bd]/30 dark:hover:border-teal-500/40 hover:bg-white dark:hover:bg-slate-800",
                "active:scale-[0.99]",
                visible && "animate-mobile-menu-item"
              )}
              style={visible ? { animationDelay: "40ms" } : undefined}
            >
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white dark:bg-slate-700/80 text-[#1461bd] dark:text-teal-400">
                <span className="material-symbols-outlined !text-[20px]">search</span>
              </span>
              <span className="flex-1 min-w-0">
                <span className="block text-[15px] font-semibold text-slate-800 dark:text-slate-100">
                  {t("header.searchMobileCta")}
                </span>
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        featured
                          ? "text-white/80"
                          : "text-slate-300 dark:text-slate-600 group-hover:text-[#1461bd] dark:group-hover:text-teal-400"
                      )}
                    >
                      chevron_right
                    </span>
