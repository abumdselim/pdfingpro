...
import { tiffFilesToPDF } from "@/lib/pdf/tiff";
...
export default function TiffToPdfPage() {
  ...
  const handleConvert = async () => {
    ...
    setError(null);
    try {
      const bytes = await tiffFilesToPDF(files);
      const blob = new Blob([bytes as unknown as BlobPart], { type: "application/pdf" });
      setResult({
        blob,
        filename: files.length === 1 ? `${getBaseName(files[0].name)}.pdf` : "tiff-images.pdf",
      });
    } catch (err: unknown) {
      setError((err as Error)?.message ?? t("tiffToPdf.error"));
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <ToolLayout
      title={t("tools.tiffToPdf.title")}
      description={t("tiffToPdf.pageDescription")}
      icon="image"
          ...
          <ToolCard>
            ...
            >
              ...
              <div className="text-center">
                <p className="font-medium text-slate-700 dark:text-slate-300">{t("tiffToPdf.dropHere")}</p>
                <p className="text-sm text-slate-500 mt-0.5">{t("tiffToPdf.formats")}</p>
              </div>
            </div>
          </ToolCard>
          ...
          {files.length > 0 && (
            <ToolCard>
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">
                {t("tiffToPdf.fileCount", { count: files.length })}
              </p>
