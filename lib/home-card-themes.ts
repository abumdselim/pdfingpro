/** Solid full-color card themes (JIT-safe static Tailwind classes). */

export interface CardTheme {
  surface: string;
  border: string;
  watermark: string;
  icon: string;
  title: string;
  arrow: string;
}

/** White text — for mid/dark saturated backgrounds */
const ON_DARK: Omit<CardTheme, "surface" | "border"> = {
  watermark: "text-white",
  icon: "bg-white/20 text-white ring-1 ring-white/30 shadow-md",
  title: "text-white",
  arrow: "bg-white/20 text-white ring-1 ring-white/30",
};

/** Dark text — for bright yellow/lime/amber backgrounds */
const ON_LIGHT: Omit<CardTheme, "surface" | "border"> = {
  watermark: "text-slate-900",
  icon: "bg-black/10 text-slate-900 ring-1 ring-black/10 shadow-md",
  title: "text-slate-900",
  arrow: "bg-black/10 text-slate-900 ring-1 ring-black/10",
};

function solid(bg: string, border: string, text: typeof ON_DARK): CardTheme {
  return { surface: bg, border, ...text };
}

export const TOOL_CARD_THEMES: Record<string, CardTheme> = {
  "merge-pdf": solid("bg-indigo-600", "border-indigo-700", ON_DARK),
  "split-pdf": solid("bg-rose-500", "border-rose-600", ON_DARK),
  "organize-pdf": solid("bg-orange-500", "border-orange-600", ON_DARK),
  "add-page-numbers": solid("bg-sky-500", "border-sky-600", ON_DARK),
  "pdf-to-jpg": solid("bg-orange-600", "border-orange-700", ON_DARK),
  "jpg-to-pdf": solid("bg-fuchsia-600", "border-fuchsia-700", ON_DARK),
  "pdf-to-word": solid("bg-blue-600", "border-blue-700", ON_DARK),
  "word-to-pdf": solid("bg-blue-700", "border-blue-800", ON_DARK),
  "pdf-to-powerpoint": solid("bg-orange-600", "border-orange-700", ON_DARK),
  "powerpoint-to-pdf": solid("bg-orange-700", "border-orange-800", ON_DARK),
  "excel-to-pdf": solid("bg-green-800", "border-green-900", ON_DARK),
  "website-to-pdf": solid("bg-cyan-600", "border-cyan-700", ON_DARK),
  "compress-pdf": solid("bg-emerald-600", "border-emerald-700", ON_DARK),
  "rotate-pdf": solid("bg-purple-600", "border-purple-700", ON_DARK),
  "watermark-pdf": solid("bg-teal-600", "border-teal-700", ON_DARK),
  "sign-pdf": solid("bg-pink-600", "border-pink-700", ON_DARK),
  "highlight-pdf": solid("bg-yellow-400", "border-yellow-500", ON_LIGHT),
  "stamp-pdf": solid("bg-lime-500", "border-lime-600", ON_LIGHT),
  "crop-pdf": solid("bg-green-600", "border-green-700", ON_DARK),
  "add-padding-pdf": solid("bg-slate-600", "border-slate-700", ON_DARK),
  "ocr-pdf": solid("bg-violet-600", "border-violet-700", ON_DARK),
  "protect-pdf": solid("bg-red-600", "border-red-700", ON_DARK),
  "unlock-pdf": solid("bg-green-500", "border-green-600", ON_DARK),
  "pdf-workflow": solid("bg-blue-700", "border-blue-800", ON_DARK),
  "compare-pdf": solid("bg-indigo-500", "border-indigo-600", ON_DARK),
  "pdf-to-excel": solid("bg-green-700", "border-green-800", ON_DARK),
  "extract-images-pdf": solid("bg-amber-600", "border-amber-700", ON_DARK),
  "pdf-to-pdfa": solid("bg-slate-700", "border-slate-800", ON_DARK),
  "pdfa-preflight": solid("bg-slate-800", "border-slate-900", ON_DARK),
  "pdf-to-pdfa-server": solid("bg-blue-800", "border-blue-900", ON_DARK),
  "redact-pdf": solid("bg-neutral-700", "border-neutral-800", ON_DARK),
  "edit-metadata-pdf": solid("bg-zinc-600", "border-zinc-700", ON_DARK),
  "repair-pdf": solid("bg-orange-700", "border-orange-800", ON_DARK),
  "fill-pdf-forms": solid("bg-cyan-700", "border-cyan-800", ON_DARK),
  "verify-signature-pdf": solid("bg-red-700", "border-red-800", ON_DARK),
  "extract-text-pdf": solid("bg-slate-500", "border-slate-600", ON_DARK),
  "pdf-to-png": solid("bg-violet-600", "border-violet-700", ON_DARK),
  "png-to-pdf": solid("bg-indigo-500", "border-indigo-600", ON_DARK),
  "reverse-pdf": solid("bg-amber-600", "border-amber-700", ON_DARK),
  "grayscale-pdf": solid("bg-neutral-600", "border-neutral-700", ON_DARK),
  "invert-pdf": solid("bg-purple-700", "border-purple-800", ON_DARK),
  "pdf-info": solid("bg-blue-500", "border-blue-600", ON_DARK),
  "flatten-pdf": solid("bg-teal-600", "border-teal-700", ON_DARK),
  "remove-blank-pages": solid("bg-sky-600", "border-sky-700", ON_DARK),
  "duplicate-pages": solid("bg-indigo-600", "border-indigo-700", ON_DARK),
  "extract-pages": solid("bg-emerald-600", "border-emerald-700", ON_DARK),
  "remove-annotations": solid("bg-rose-600", "border-rose-700", ON_DARK),
  "add-image-pdf": solid("bg-pink-600", "border-pink-700", ON_DARK),
  "resize-pdf": solid("bg-cyan-600", "border-cyan-700", ON_DARK),
  "n-up-pdf": solid("bg-violet-600", "border-violet-700", ON_DARK),
  "booklet-pdf": solid("bg-amber-600", "border-amber-700", ON_DARK),
  "sanitize-pdf": solid("bg-slate-600", "border-slate-700", ON_DARK),
  "compress-images-pdf": solid("bg-emerald-600", "border-emerald-700", ON_DARK),
  "split-by-bookmarks": solid("bg-rose-600", "border-rose-700", ON_DARK),
  "bookmarks-pdf": solid("bg-amber-600", "border-amber-700", ON_DARK),
  "header-footer-pdf": solid("bg-sky-600", "border-sky-700", ON_DARK),
  "webp-to-pdf": solid("bg-lime-600", "border-lime-700", ON_DARK),
  "split-by-size": solid("bg-rose-600", "border-rose-700", ON_DARK),
  "merge-alternate": solid("bg-indigo-600", "border-indigo-700", ON_DARK),
  "add-links-pdf": solid("bg-blue-600", "border-blue-700", ON_DARK),
  "text-to-pdf": solid("bg-stone-600", "border-stone-700", ON_DARK),
  "auto-rotate-pdf": solid("bg-purple-600", "border-purple-700", ON_DARK),
  "extract-attachments-pdf": solid("bg-amber-600", "border-amber-700", ON_DARK),
  "tiff-to-pdf": solid("bg-cyan-600", "border-cyan-700", ON_DARK),
  "visual-diff-pdf": solid("bg-rose-600", "border-rose-700", ON_DARK),
  "batch-workflow": solid("bg-blue-800", "border-blue-900", ON_DARK),
  "add-attachments-pdf": solid("bg-teal-600", "border-teal-700", ON_DARK),
  "heic-to-pdf": solid("bg-violet-600", "border-violet-700", ON_DARK),
  "bates-numbering-pdf": solid("bg-slate-700", "border-slate-800", ON_DARK),
  "pdf-to-html": solid("bg-orange-600", "border-orange-700", ON_DARK),
  "split-by-text": solid("bg-indigo-600", "border-indigo-700", ON_DARK),
  "csv-to-pdf": solid("bg-green-700", "border-green-800", ON_DARK),
  "pdf-to-csv": solid("bg-green-800", "border-green-900", ON_DARK),
  "pdf-summary": solid("bg-violet-700", "border-violet-800", ON_DARK),
  "create-pdf-form": solid("bg-cyan-700", "border-cyan-800", ON_DARK),
  "markdown-to-pdf": solid("bg-slate-600", "border-slate-700", ON_DARK),
  "bmp-to-pdf": solid("bg-fuchsia-600", "border-fuchsia-700", ON_DARK),
  "gif-to-pdf": solid("bg-pink-600", "border-pink-700", ON_DARK),
  "insert-pages-pdf": solid("bg-indigo-600", "border-indigo-700", ON_DARK),
  "overlay-pdf": solid("bg-teal-700", "border-teal-800", ON_DARK),
  "pdf-to-markdown": solid("bg-stone-600", "border-stone-700", ON_DARK),
  "json-to-pdf": solid("bg-amber-700", "border-amber-800", ON_DARK),
  "add-blank-pages": solid("bg-sky-600", "border-sky-700", ON_DARK),
  "shuffle-pdf": solid("bg-purple-700", "border-purple-800", ON_DARK),
  "mirror-pdf": solid("bg-rose-700", "border-rose-800", ON_DARK),
  "posterize-pdf": solid("bg-orange-700", "border-orange-800", ON_DARK),
  "fit-to-page-pdf": solid("bg-cyan-800", "border-cyan-900", ON_DARK),
  "page-labels-pdf": solid("bg-blue-600", "border-blue-700", ON_DARK),
  "pdf-to-json": solid("bg-yellow-600", "border-yellow-700", ON_LIGHT),
  "tile-pdf": solid("bg-lime-600", "border-lime-700", ON_DARK),
  "replace-pages-pdf": solid("bg-indigo-800", "border-indigo-900", ON_DARK),
  "add-background-pdf": solid("bg-emerald-700", "border-emerald-800", ON_DARK),
  "pdf-validator": solid("bg-slate-800", "border-slate-900", ON_DARK),
  "xml-to-pdf": solid("bg-orange-800", "border-orange-900", ON_DARK),
  "split-even-odd": solid("bg-rose-800", "border-rose-900", ON_DARK),
  "export-bookmarks-pdf": solid("bg-amber-800", "border-amber-900", ON_DARK),
  "qr-code-pdf": solid("bg-neutral-700", "border-neutral-800", ON_DARK),
  "invoice-pdf": solid("bg-teal-800", "border-teal-900", ON_DARK),
  "linearize-pdf": solid("bg-blue-800", "border-blue-900", ON_DARK),
  "duplicate-range-pdf": solid("bg-pink-700", "border-pink-800", ON_DARK),
  "svg-to-pdf": solid("bg-indigo-900", "border-indigo-950", ON_DARK),
  "sort-pages-size-pdf": solid("bg-violet-800", "border-violet-900", ON_DARK),
  "pdf-to-pdfa-3": solid("bg-slate-900", "border-slate-950", ON_DARK),
  "combine-text-pdf": solid("bg-stone-700", "border-stone-800", ON_DARK),
  "remove-metadata-pdf": solid("bg-red-800", "border-red-900", ON_DARK),
  "split-every-n-pages": solid("bg-rose-600", "border-rose-700", ON_DARK),
};

export function getToolCardTheme(toolId: string): CardTheme {
  return TOOL_CARD_THEMES[toolId] ?? TOOL_CARD_THEMES["merge-pdf"];
}
